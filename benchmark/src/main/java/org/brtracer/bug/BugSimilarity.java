...
public class BugSimilarity {
	private int wordCount = BroccoliContext.getInstance().getContextVar("BugTermCount");
	private int bugReportCount = BroccoliContext.getInstance().getContextVar("BugReportCount");
	...
	public void computeSimilarity() throws IOException {
		...
		for (int i = 0; i < bugReportCount; i++) {
			...
			writer.write(output.trim() + BroccoliContext.getInstance().getLineSeparator());
