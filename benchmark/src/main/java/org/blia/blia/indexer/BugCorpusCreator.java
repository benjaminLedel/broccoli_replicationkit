 ...
 */
package org.blia.blia.indexer;

import de.broccoli.context.BroccoliContext;
import org.blia.Property;
import org.blia.common.Bug;
import org.blia.common.BugCorpus;
import org.blia.db.dao.BugDAO;
import org.blia.db.dao.SourceFileDAO;
import org.blia.utils.Splitter;
import org.blia.utils.Stem;
import org.blia.utils.Stopword;
...
//        String pattern = "(([a-zA-Z0-9_\\-$]*\\.)*[a-zA-Z_<][a-zA-Z0-9_\\-$>]*\\(([a-zA-Z_][a-zA-Z0-9_\\-]*\\.java:[0-9]*|(?i)native method|(?i)unknown source)\\))";
	...
	private ArrayList<Bug> parseXML(boolean stackTraceAnalysis) {
		...
		ArrayList<de.broccoli.dataimporter.models.Bug> bugImporter = BroccoliContext.getInstance().getBugList();
		for (de.broccoli.dataimporter.models.Bug singleBug: bugImporter) {
			Bug bug = new Bug();
			bug.setID(Integer.parseInt(singleBug.getBugId()));
			bug.setOpenDate(singleBug.getOpenDate());//simpleDateFormat.parse(openDateString));
			bug.setFixedDate(singleBug.getFixDate());//simpleDateFormat.parse(fixDateString));

			bug.setSummary(singleBug.getBugSummary());

			//
			String content = singleBug.getBugDescription();
			...
			for (String fixedFile: singleBug.getSet()) {
				String checkingString = "org.aspectj/modules/";
				String fileName = fixedFile;
				if (fixedFile.contains(checkingString)) {
