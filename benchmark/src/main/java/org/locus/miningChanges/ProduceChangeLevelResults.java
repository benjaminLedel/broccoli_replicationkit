...
public class ProduceChangeLevelResults {
	...
	public boolean loadOracles() {
		String filename = org.locus.main.Main.getInstance().changeOracle;
...
//			System.out.println(lines.get(index));
	...
	public void loadRevisionTime() throws ParseException {
		List<String> lines = FileToLines.fileToLines(de.broccoli.context.BroccoliContext.getInstance().getWorkDir()  + File.separator + "logOneline.txt");
...
//			System.out.println(line);
	...
	public void integrateResults() {
		...
		double belta2 = Main.getInstance().belta2;
		for (Bug bug : bugs) {
			...
			for (String change : results.keySet()) {
				if (revisionTime.containsKey(change) && revisionTime.get(change) < bug.getOpenDate().getTime())
...
//				System.out.println(change + "\t" + revisionTime.get(change));
		...
		System.out.println();
		String filename = de.broccoli.context.BroccoliContext.getInstance().getWorkDir()  + File.separator + "changeLevelResults.txt";
