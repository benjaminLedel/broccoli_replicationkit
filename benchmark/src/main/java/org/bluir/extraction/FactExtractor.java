...
import org.bluir.core.GenericVisitorFact;
import org.bluir.utility.FileDetector;
import org.bluir.utility.FileSystem;
import org.bluir.utility.PreProcessor;
import org.bluir.utility.StructureDiffUtils;
...
//
	...
	public static boolean extractEclipseFacts(String codeDirectory, String destinationDirectory) throws IOException, CoreException
	{
		...
		BufferedWriter bwIndex = new BufferedWriter(new FileWriter(BroccoliContext.getInstance().getWorkDir() + BroccoliContext.getInstance().getSeparator() + "BLUIR" + BroccoliContext.getInstance().getSeparator() + "FileIndex.txt"));
		...
		for (File srcFile : files){
			...
			if (packageNameObj!=null && !BroccoliContext.getInstance().getProjectName().startsWith("ASPECTJ"))
		...
		BroccoliContext.getInstance().setContextVar("fileCount",fileCount);
		return true;
	}
	...
	public static void extractFactsDefault(String codeDirectory, String destinationDirectory) throws IOException, CoreException {
		...
		BufferedWriter bwIndex = new BufferedWriter(new FileWriter(BroccoliContext.getInstance().getWorkDir() + BroccoliContext.getInstance().getSeparator() + "FileIndex.txt"));
